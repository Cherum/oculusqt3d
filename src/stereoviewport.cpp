...
/*!
    ...
    \sa paint()
*/
void StereoViewport::beforeRendering()
{
...
void StereoViewport::render(QGLPainter *painter)
{
...
//                           viewport.height() );
        ...
        if (d->stereoType != StereoViewport::Hardware) {
            earlyDraw(painter);     // Clear both eyes at the same time.
            // May've been set by early draw
            glDisable(GL_CULL_FACE);
        }
        ...
        // May've been set by one of the items
        glDisable(GL_CULL_FACE);
        if (d->stereoType == StereoViewport::RedCyanAnaglyph) {
            glClear(GL_DEPTH_BUFFER_BIT);
        }
...
/*!
  \internal
*/
void StereoViewport::earlyDraw(QGLPainter *painter)
{
    painter->disableEffect();
...
/*!
  ...
  been populated.
*/
void StereoViewport::draw(QGLPainter *painter)
{
...
void StereoViewport::initializeGL(QGLPainter *painter)
{
...
void StereoViewport::registerEarlyDrawObject(QObject *obj, int order)
{
    d->earlyDrawList.insertMulti(order, obj);
}

PickEvent *StereoViewport::initiatePick(QMouseEvent *pick)
{
...
*/
void StereoViewport::setupPickPaint(QGLPainter *painter, const QPointF &pt)
{
    painter->setPicking(true);
    painter->clearPickObjects();
    d->setRenderSettings(painter);
...
/*!
    \internal
*/
void StereoViewport::cameraChanged()
{
    update3d();
}
...
/*!
    \internal
*/
void StereoViewport::mousePressEvent(QMouseEvent *e)
{
...
void StereoViewport::processMousePress(PickEvent *pick)
{
...
void StereoViewport::processNavEvent(QMouseEvent *e)
{
    d->panning = true;
...
/*!
    \internal
*/
void StereoViewport::mouseReleaseEvent(QMouseEvent *e)
{
...
void StereoViewport::processMouseRelease(PickEvent *pick)
{
...
/*!
    \internal
*/
void StereoViewport::mouseDoubleClickEvent(QMouseEvent *e)
{
...
void StereoViewport::processMouseDoubleClick(PickEvent *pick)
{
    QObject *object = pick->object();
    QMouseEvent *e = pick->event();
    if (object) {
...
*/
bool StereoViewport::mouseMoveOverflow(QMouseEvent *e) const
{
    bool result = false;
    QMutexMaybeLocker locker(&d->pickEventQueueLock);
    if (d->pickEventQueue.size() > 0)
...
/*!
    \internal
*/
void StereoViewport::mouseMoveEvent(QMouseEvent *e)
{
...
void StereoViewport::processMouseMove(PickEvent *pick)
{
    QObject *object = pick->object();
    QMouseEvent *e = pick->event();
    if (d->pressedObject) {
...
/*!
    \internal
*/
void StereoViewport::hoverEnterEvent(QHoverEvent *e)
{
    if (hoverEvent(e))
        e->setAccepted(true);
    else
...
/*!
    \internal
*/
void StereoViewport::hoverMoveEvent(QHoverEvent *e)
{
    if (hoverEvent(e))
        e->setAccepted(true);
    else
...
/*!
    \internal
*/
void StereoViewport::hoverLeaveEvent(QHoverEvent *e)
{
...
/*!
    \internal
*/
void StereoViewport::wheelEvent(QWheelEvent *e)
{
    if (d->navigation) {
        wheel(e->delta());
        e->setAccepted(true);
...
/*!
    \internal
*/
void StereoViewport::keyPressEvent(QKeyEvent *e)
{
...
/*!
    \internal
*/
bool StereoViewport::hoverEvent(QHoverEvent *e)
{
...
void StereoViewport::processMouseHover(PickEvent *pick)
{
    QObject *object = pick->object();
    QMouseEvent *e = pick->event();
...
void StereoViewport::wheel(float delta)
{
    if (d->fovzoom) {
...
void StereoViewport::pan(float deltax, float deltay)
{
...
void StereoViewport::rotate(float deltax, float deltay)
{
    QRectF rect = boundingRect();
...
QPointF StereoViewport::viewDelta(float deltax, float deltay)
{
    QRectF rect = boundingRect();
    float w = rect.width();
    float h = rect.height();
...
void StereoViewport::itemChange(QQuickItem::ItemChange change, const ItemChangeData &value)
{
...
QSGNode* StereoViewport::updatePaintNode(QSGNode* node, UpdatePaintNodeData* data)
{
    Q_UNUSED(node);
    Q_UNUSED(data);
    if (d->renderMode == BufferedRender)
...
void StereoViewport::sceneGraphInitialized()
{
...
void StereoViewport::geometryChanged(const QRectF &newGeometry, const QRectF &oldGeometry)
{
    QQuickItem::geometryChanged(newGeometry, oldGeometry);
    setSize(newGeometry.size());
}

void StereoViewport::canvasDeleted()
{
    d->canvas = 0;
}
